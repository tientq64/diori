...
export function registerCompletionItemProvider(): Monaco.IDisposable {
	...
	const disposer: Monaco.IDisposable = monaco.languages.registerCompletionItemProvider(
		'diori',
		{
			provideCompletionItems(model, position) {
				...
				const range2 = range.setEndPosition(
					range.endLineNumber,
					range.endColumn - 1
				)
				const suggestions = entities.map<Monaco.languages.CompletionItem>(
					(entity) => {
